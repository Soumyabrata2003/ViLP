...
class Video_dataset(data.Dataset):
    def __init__(self, root_path, list_file, labels_file, pose_root,pose_list,
    ...
    def __getitem__(self, index):
        ...
        if self.modality == 'video':
            ...
            npy_list= os.listdir(os.path.join(self.pose_root,record.path))  # pose 
        ...
        return self.get(record, video_list,npy_list, segment_indices)
    ...
    def _load_pose(self, directory, idx):
        if self.modality == 'RGB':
            try:
                pose_jpg=self.image_tmpl.format(idx)
                pose_npy=pose_jpg.split('.')[0] +'.npy'
                d=np.load(os.path.join(self.pose_root, directory, pose_npy))
                d=d*255
                return [Image.fromarray(d).convert('RGB')]
            except Exception:
                pose_jpg=self.image_tmpl.format(idx)
                pose_npy=pose_jpg.split('.')[0] +'.npy'
                print('error loading image:', os.path.join(self.pose_root, directory, pose_npy))
                pose_jpg=self.image_tmpl.format(1)
                pose_npy=pose_jpg.split('.')[0] +'.npy'
                d=np.load(os.path.join(self.pose_root, directory, pose_npy))
                d=d*255
                return [Image.fromarray(d).convert('RGB')]
    ...
    def get(self, record, video_list, npy_list, indices):
        images = list()
        poses= list()
        for seg_ind in indices:
            ...
            if self.modality == 'video':
                ...
                seg_pose = self._load_pose(record.path, p)
                # print(type(seg_pose))
        ...
        if self.num_sample > 1:
            ...
            for _ in range(self.num_sample):
                ...
                pose_data, record_label =self.transform((poses, record.label))
                frame_list.append(process_data)
                label_list.append(record_label)
                poses_list.append(pose_data)
            return frame_list, poses_list, label_list
        else:
            process_data, record_label = self.transform((images, record.label))
            pose_data, record_label= self.transform((poses, record.label))
            return process_data, pose_data, record_label
