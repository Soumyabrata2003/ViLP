    ...
    for i,(images, poses, list_id) in enumerate(train_loader):
        ...
        with autocast():
            if config.solver.loss_type in ['NCE', 'DS']:
                ...
                text_embedding=torch.ones(text_embedding.shape)
                # cls_embedding=torch.ones(cls_embedding.shape)

                logits = ((logit_scale+logit_scale_pose)/2) * video_head(image_embedding, pose_embedding, text_embedding, cls_embedding) # passed pose embed as well
...
def validate(epoch, val_loader, classes, device, model, model_pose, video_head, config, n_class, logger, return_sim=False):
    ...
    with torch.no_grad():
        ...
        text_features=torch.ones(text_features.shape)
        for i, (image, pose, class_id) in enumerate(val_loader):
            ...
            similarity = video_head(image_features, pose_features, text_features, cls_feature)
