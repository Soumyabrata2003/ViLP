...
class video_header(nn.Module):
    ...
    def get_logits(self, vid_emb, pose_emb, text_emb, cls_emb):
        vid_emb = self.agg_video_feat(vid_emb)  # b t c
        # pose_emb = self.agg_video_feat(pose_emb)  # b t c
        ...
        if self.interaction == 'DP':
            ...
            cls_emb = cls_emb / cls_emb.norm(dim=-1, keepdim=True)
            vid_emb = vid_emb / vid_emb.norm(dim=-1, keepdim=True)
            pose_emb = pose_emb / pose_emb.norm(dim=-1, keepdim=True)
            text_emb = text_emb / text_emb.norm(dim=-1, keepdim=True)
            vid_pose = torch.mul(vid_emb,torch.sigmoid(pose_emb))  # pose attention to vid_emb
            sims = torch.einsum('awd,btd->abwt', [text_emb, vid_pose])
            att_weight_v = F.softmax(sims/0.01, dim=-1) # abwt
            att_weight_v = att_weight_v.mean(dim=-2)  # abt
            v_att = torch.einsum('abt,btd->abd', [att_weight_v, vid_pose])
            # new
    ...
    def forward(self, vid_emb, pose_emb, text_emb, cls_emb):
        logits = self.get_logits(vid_emb, pose_emb, text_emb, cls_emb)
        return logits
