...
def _optimizer(config, model,model_pose, video_head):
    if config.solver.optim == 'adam':
        ...
        for name, param in model_pose.named_parameters():
            if 'visual.' in name:
                vision_pose_params.append(param)
            else:
                text_pose_params.append(param)        
        ...
        optimizer = optim.AdamW([{'params': model.parameters(), 'lr': config.solver.lr * config.solver.clip_ratio},
                                 {'params': model_pose.parameters(), 'lr': config.solver.lr * config.solver.clip_ratio},
                                 {'params': video_head.parameters(), 'lr': config.solver.lr}],
