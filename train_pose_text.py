...
from modules.video_clip2 import video_header
...
def main(args):
    ...
    model_pose, clip_state_dict_pose = clip.load(
        config.network.arch,
        device='cpu',jit=False,
        internal_modeling=config.network.tm,
        T=config.data.num_segments,
    ...
    if config.data.dataset == 'charades':
        ...
        train_data = Video_dataset(        # Included pose root and list in dataset object
            config.data.train_root, config.data.train_list,
            config.data.label_list, num_segments=config.data.num_segments,
            modality=config.data.modality,
            image_tmpl=config.data.image_tmpl, random_shift=config.data.random_shift,
            transform=transform_train, dense_sample=config.data.dense,pose_root=config.data.pose_root,pose_list=config.data.pose_list)
        val_data = Video_dataset(
            config.data.val_root, config.data.val_list, config.data.label_list,
            random_shift=False, num_segments=config.data.num_segments,
            modality=config.data.modality,
            image_tmpl=config.data.image_tmpl,
            transform=transform_val, dense_sample=config.data.dense,pose_root=config.data.pose_val_root,pose_list=config.data.pose_val_list)            
    ...
    if config.pretrain:
        if os.path.isfile(config.pretrain):
            ...
            # model_pose.load_state_dict(checkpoint['model_pose_state_dict'])
    ...
    if config.resume:
        if os.path.isfile(config.resume):
            ...
            model_pose.load_state_dict(update_dict(checkpoint['model_pose_state_dict']))
    ...
    # for model_pose
    if config.network.fix_text:
        for name, param in model_pose.named_parameters():
            if "visual" not in name and "logit_scale" not in name:
                param.requires_grad_(False)
    ...
    optimizer = _optimizer(config, model,model_pose, video_head)
    lr_scheduler = _lr_scheduler(config, optimizer)

    if args.distributed:
        model = DistributedDataParallel(model.cuda(), device_ids=[args.gpu], find_unused_parameters=False)
        model_pose = DistributedDataParallel(model_pose.cuda(), device_ids=[args.gpu],find_unused_parameters=False)
    ...
    if config.solver.evaluate:
        ...
        if config.data.dataset == 'charades':
            prec1, output_list, labels_list = validate_mAP(
                ...
                model, model_pose, video_head, config, n_class, logger)
        else:
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        train(model, model_pose, video_head, train_loader, optimizer, criterion, scaler,
              epoch, device, lr_scheduler, config, classes, logger)

        if (epoch+1) % config.logging.eval_freq == 0:
            if config.data.dataset == 'charades':
                prec1, output_list, labels_list = validate_mAP(epoch, val_loader, classes, device, model, model_pose, video_head, config, n_class, logger)
            else:
                prec1, output_list, labels_list = validate(epoch, val_loader, classes, device, model, model_pose, video_head, config, n_class, logger, save_score)
            ...
            if dist.get_rank() == 0:
                ...
                epoch_saving(epoch, model.module, model_pose.module, video_head_nomodule, optimizer, filename)
                if is_best:
                    best_saving(working_dir, epoch, model.module, model_pose.module, video_head_nomodule, optimizer)
                    if save_score:
                        save_sims(output_list, labels_list)



def train(model, model_pose, video_head, train_loader, optimizer, criterion, scaler,
          epoch, device, lr_scheduler, config, classes, logger):
    """ train a epoch """
    ...
    for i,(images, poses, list_id) in enumerate(train_loader):
        ...
        poses = poses.view((-1,config.data.num_segments,3)+poses.size()[-2:])  # bt 3 h w
        b1,t1,c1,h1,w1 = poses.size()

        poses= poses.view(-1,c1,h1,w1) 
        ...
        with autocast():
            if config.solver.loss_type in ['NCE', 'DS']:
                ...
                pose_embedding, cls_embedding_pose, text_embedding_pose, logit_scale_pose = model_pose(poses, texts, return_token=True)
                pose_embedding = pose_embedding.view(b1,t1,-1)  # incorporated pose here
                # gather
                pose_embedding = allgather(pose_embedding)
                # if text_embedding is not None:
                #     text_embedding = allgather(text_embedding)
                # cls_embedding = allgather(cls_embedding) 

                if text_embedding_pose is not None:
                    text_embedding_pose = allgather(text_embedding_pose)
                cls_embedding_pose = allgather(cls_embedding_pose)     
                
                # text_embedding=(text_embedding+text_embedding_pose)/2  #dummy sums to mitigate the problem of unused params
                # cls_embedding=(cls_embedding+cls_embedding_pose)/2

                logits = (logit_scale_pose) * video_head(pose_embedding, text_embedding_pose, cls_embedding_pose) # passed pose embed as well
                
                list_id = gather_labels(list_id.to(device))  # bs -> n_gpu * bs

                ground_truth = torch.tensor(gen_label(list_id),dtype=pose_embedding.dtype,device=device)
                # gt = [bs bs]
...
def validate(epoch, val_loader, classes, device, model, model_pose, video_head, config, n_class, logger, return_sim=False):
    top1 = AverageMeter()
    top5 = AverageMeter()
    sims_list = []
    labels_list = []
    ...
    with torch.no_grad():
        ...
        for i, (image, pose, class_id) in enumerate(val_loader):
            # image = image.view((-1, config.data.num_segments, 3) + image.size()[-2:])
            # b, t, c, h, w = image.size()
            class_id = class_id.to(device)
            # image_input = image.to(device).view(-1, c, h, w)
            # image_features = model.module.encode_image(image_input).view(b, t, -1)
            pose = pose.view((-1, config.data.num_segments, 3) + pose.size()[-2:]) #pose component
            b1, t1, c1, h1, w1 = pose.size()
            # class_id = class_id.to(device)
            pose_input = pose.to(device).view(-1, c1, h1, w1)
            pose_features = model_pose.module.encode_image(pose_input).view(b1, t1, -1)
            similarity = video_head( pose_features, text_features, cls_feature)

            similarity = similarity.view(b1, -1, n_class).softmax(dim=-1)  # [bs, n_frames, n_cls]
...
def save_sims(output_list, labels_list):
    ...
    torch.save(outputs_sim, '/home/mtech/project_env/VLP/video_sentence_fusion/k400_video_sims.pt')
    torch.save(labels_list_res, '/home/mtech/project_env/VLP/video_sentence_fusion/k400_video_labels.pt')
