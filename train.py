    ...
    for i,(images, poses, list_id) in enumerate(train_loader):
        ...
        with autocast():
            if config.solver.loss_type in ['NCE', 'DS']:
                ...
                image_embedding, pose_embedding, cls_embedding, text_embedding, logit_scale = model(images, poses, texts, return_token=True)
                ...
                logits = logit_scale * video_head(image_embedding, pose_embedding, text_embedding, cls_embedding) # passed pose embed as well
...
def validate(epoch, val_loader, classes, device, model, video_head, config, n_class, logger, return_sim=False):
    ...
    with torch.no_grad():
        ...
        for i, (image, pose, class_id) in enumerate(val_loader):
            ...
            pose_features = model.module.encode_pose(pose_input).view(b1, t1, -1)
...
def save_sims(output_list, labels_list):
    ...
    torch.save(outputs_sim, '/home/mtech/project_env/BIKE/video_sentence_fusion/k400_video_sims.pt')
    torch.save(labels_list_res, '/home/mtech/project_env/BIKE/video_sentence_fusion/k400_video_labels.pt')
